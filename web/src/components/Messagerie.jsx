...
const menue = require('../img/menu.png')
const profile = require('../img/wp7710993-miles-morales-4k-ps5-wallpapers.jpg')
...
function Messenger() {
    ...
    const test =[
        {status:true,message:"bonjour randry il fait beau"},
        {status:false,message:"oui tu as raison il fait beau"},
        {status:true,message:"tu compte sortir aujourdhui ?"},
        {status:false,message:"non je ne pense pas"}
    ]

    // comportement

    function handlecharge_message(){
        document.getElementById("block_message").animate([
        ...
        setTimeout(() => {
            ...
            document.getElementById("block_discussion").style.display = "block";
        }, 1000);
    }


    function enable_dashboard() {
        fetch("http://127.0.0.1:2000/Messagerie/read")
            .then((res) => res.json()
            )
            .then((json) => {
                setitem({
                    items: json[0].DISCUSSION,
                });
                // for(let i=0;i<json.length;i++){
                //     for(let j = 0;j<json.length;j++){
                //         console.log(json[i].DISCUSSION[j].MESSAGES)
                //     }
                // }
            })
    }
    ...
    return (
        <>
            <div id="messagerie_container">
                <section id="header">
                    <section id="sous_header">
                        <img id="logo_header" src={menue}></img>
                        <section id="title_header">DISCUSSIONS</section>
                    </section>
                </section>
                <section id="discussion">
                    {
                        item.items.map((e) => (
                            <>
                                <section className="item_message" onClick={()=>{
                                    handlecharge_message()
                                }}>
                                    <img className="profile_discuss" src={profile}></img>
                                    <p className="block_mess">
                                        <p className="username">randry claude</p>
                                        <p className="writing">hello comment tu vas</p>
                                    </p>
                                </section>
                            </>
                        ))
                ...
                <section id="message">
                    <section id="block_message">
                    <h1 id="message_title_icon">BIENVENUE SUR L'ESPACE MESSAGERIE 😊</h1>
                    <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                        width="98px" height="100px" viewBox="0 0 98 100" enable-background="new 0 0 98 100">
  ...
  L65.877,68.221z">
                    ...
                    <section id="block_discussion">
                        {
                            item.items.map((e)=>{
                                if(e.status){
                                    return <>
                                    <section className="interlocuteur">
                                        <div className="mes_int">
                                            {e.message}
                                        </div>
                                    </section>
                                    </>
